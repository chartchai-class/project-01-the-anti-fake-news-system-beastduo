    ...
    <div class="mx-auto max-w-3xl px-4 py-8">
      ...
      <div v-else class="rounded-xl border-2 border-[#002b5c]/10 bg-white p-6 shadow-lg">
        ...
        <div class="mt-6">
          ...
          <nav v-if="totalPages > 1" class="mt-4 flex items-center justify-center gap-2">
            <button @click="goPage(page-1)" :disabled="page <= 1" class="rounded px-2 py-1 text-sm font-bold border border-[#002b5c] text-[#002b5c] bg-white disabled:opacity-50">Prev</button>
            <span class="text-sm font-bold text-[#002b5c]">Page {{ page }} of {{ totalPages }}</span>
            <button @click="goPage(page+1)" :disabled="page >= totalPages" class="rounded px-2 py-1 text-sm font-bold border border-[#e10600] text-[#e10600] bg-white disabled:opacity-50">Next</button>
          </nav>
        </div>
      </div>
    </div>
